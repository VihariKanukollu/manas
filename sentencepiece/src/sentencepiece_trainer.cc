...
util::Status SentencePieceTrainer::PopulateModelTypeFromString(
  ...
  static const std::unordered_map<std::string, TrainerSpec::ModelType>
      kModelTypeMap = {{"unigram", TrainerSpec::UNIGRAM},
                       ...
                       // Alias: treat "dsl" as a WORD-level model. This makes each
                       // whitespace-separated DSL token (e.g., "INT_7", "REAL_VAR_0")
                       // an atomic unit, avoiding any subword splitting.
                       {"dsl", TrainerSpec::WORD},
